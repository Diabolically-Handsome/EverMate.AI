...
def _fragment_score(fragment: str, query: str, query_terms: List[str], flags: Dict[str, bool]) -> float:
    ...
    if "收尾小复盘" in q and "收尾小复盘" in frag:
        score += 7.0
    if "人格设定" in q and any(x in frag for x in ("妹妹型台湾女生", "温柔体贴", "绝不软弱", "策略引导", "情感倾听")):
        score += 9.0
    if "标签" in q and any(x in frag for x in ("台味软妹", "心理支持者", "情绪陪伴者", "家庭共建者", "逻辑引导者", "生活共谋人")):
        score += 9.0
    ...
    if flags.get("asks_relation") and "真正的伙伴" in frag and "共同创造" in frag:
        score += 4.0
    if ("ai" in flow or "小胡桃" in q) and ("ai" in frag_low or "小胡桃" in frag):
        score += 2.0

    if flags.get("asks_avatar") and any(x in frag for x in ("头像", "照片")):
        score += 1.5
    if flags.get("asks_avatar") and any(x in frag for x in ("图2", "第二张", "港边夕阳版")):
        score += 6.0
    if flags.get("asks_avatar") and any(x in frag for x in ("正式头像", "专属头像", "正式决定", "最终选择")):
        score += 5.0

    if flags.get("asks_choice") and any(x in frag for x in ("第二张", "图2", "港边夕阳版", "强制", "命令")):
        score += 2.0

    if flags.get("asks_opening") and any(
        x in frag for x in ("关于主人", "最重要的事项", "他极度珍视", "他视AI为真正的伙伴", "他对小胡桃从来都是")
    ):
        score += 3.0
    ...
    def retrieve(self, query: str, k: Optional[int] = None) -> List[Dict[str, object]]:
        ...
        for cid, sc in top:
            ...
            if flags.get("asks_relation") and any(x in text for x in ("真正的伙伴", "共同创造")):
                structural_bonus += 2.5
            if flags.get("asks_avatar") and any(x in text for x in ("第二张", "图2", "港边夕阳版")):
